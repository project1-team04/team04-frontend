...
//   { id: 1, issueName: '로그인 오류 발생', assignee: '양혜림' },
...
const Search = () => {
  return (
    <div className='flex h-full flex-col items-center overflow-hidden'>
      ...
      {/* 검색 결과 영역 - 스크롤 적용 */}
      <div className='my-3 h-full w-full divide-y divide-divider-default overflow-y-auto rounded-lg border border-gray bg-gray'>
        {mockSearchResults.length > 0 ? (
          mockSearchResults.map((issue) => (
            <div key={issue.id} className='bg-white px-4 py-2'>
              <p className='text-sm font-medium'>{issue.issueName}</p>
              <p className='text-xs text-text-sub'>{issue.assignee}</p>
            </div>
          ))
        ) : (
